...
89. **新增「跨境堡批量投保」：系统货箱清单按 SO 聚合填充跨境堡箱单模板** (2026-09-16)
    - 功能入口 `/kuajingbao-insurance`：上传系统导出的货箱清单，按「系统SO」聚合生成 36 列投保数据。核心 4 字段按真实填好的模板示例逐格反推得出，与需求 docx 粗略示例不同：
      ...
      2. **备注（仅「全美锁仓」渠道）** 精确文案：`本保单承保AGL（亚马逊物流）承运的物流运输服务，其亚马逊货件建仓地址为“${仓库代码}”，投保的目的地为“${仓库代码}”，同时存在最后FBA调仓可能`——仓库代码只出现在前两处（中文全角引号 `“”` 包裹），结尾「同时存在最后FBA调仓可能」**不带仓库代码**。
...
91. **跨境堡批量投保：货箱重量「合并单元格被乘 N 倍」+ 备注多了一个仓库代码** (2026-09-17)
    - 对照 `【正确写法】-YXMY26090063_跨境堡投保箱单.xlsx` 发现两处与正确答案不一致：
      1. **总公斤数（毛重）被乘 N 倍**：货箱清单里「货箱重量」列（以及「系统SO」「体积」列）是**每 SO 合并单元格**（如 `J3:J8`、`J9:J20`），合并格的值只存在主格（首行）。但 ExcelJS 读合并格时**主格值在合并区的每一行都会返回**（`getCell(r, J)` 对 r=3..8 都返回 18.95），旧代码 `g.totalWeight += cellNum(...)` 逐行累加 → 总公斤数 = 单值 × 行数（如 SO 48 变成 18.95×6=113.7，正确应为 18.95）。**修复**：把「货箱重量」从逐行累加改为**建组时只取首行一次**（与 `仓库代码`/`国家`/`客户渠道` 等每 SO 字段一致）。⚠️ 「总申报货值」「总箱数」是**逐行值**（非合并），仍需逐行累加——`总箱数` 可能同一 SO 内多行各为 1（如 SO 49 两行各 1 → 总包装数量 2），不能一概「取一次」。
      2. **备注多了一个仓库代码**：旧文案结尾是 `…同时存在最后FBA调仓可能${仓库代码}`，正确答案结尾就是 `…同时存在最后FBA调仓可能`（**不带仓库代码**）——仓库代码只出现在「建仓地址为“X”」「投保的目的地为“X”」两处。**修复**：去掉结尾的 `${仓库代码}`。
