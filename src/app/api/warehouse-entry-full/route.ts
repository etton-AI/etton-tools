/**
 * POST /api/warehouse-entry-full — 上传客户数据 + 供应商数据，返回建议结果（逐箱版）
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `📦 TR全量入仓: 客户=${customerFile.name} (${(customerFile.size / 1024).toFixed(1)} KB), 供应商=${supplierFile.name} (${(supplierFile.size / 1024).toFixed(1)} KB)`
    );
    ...
    try {
      ...
      const { rows, supplierTotal } = buildSuggestions(customers, boxes);
